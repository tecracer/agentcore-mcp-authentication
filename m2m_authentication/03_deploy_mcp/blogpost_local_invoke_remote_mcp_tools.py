...
on the deployed MCP server using Bearer token authentication from Cognito M2M flow.

This is Step 5 of the complete workflow:
1. python setup_M2M_cognito.py setup --mcp-name <name>  (Set up Cognito)
...
5. python blogpost_invoke_mcp_tools.py --name <name>    (Test deployed server) <- YOU ARE HERE

Prerequisites:
    - Steps 1-4 completed successfully
    - MCP server deployed to AgentCore Runtime
    - Cognito credentials stored in SSM Parameter Store
...
def get_cognito_bearer_token(mcp_name):
    ...
    print("Getting Bearer token from Cognito...")
    ...
    try:
        ...
        domain_url = ssm_client.get_parameter(
        ...
        print(f"Retrieved credentials from SSM")
        print(f"Using Cognito domain: {domain_url}")
        ...
        data = {
            ...
            'scope': 'blogpost-m2m-resource-server/read'
        }
...
async def main():
    ...
    if not bearer_token:
        ...
        print(f"Make sure you completed Step 1: python setup_M2M_cognito.py setup --mcp-name {mcp_name}")
        print("This stores the required credentials in AWS SSM Parameter Store")
        sys.exit(1)
